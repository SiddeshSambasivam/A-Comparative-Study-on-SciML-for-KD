...
def render_algebraic_page():
    ...
    if st.button("Train model"):
        ...
        with col1:
            st.metric(label="Prediction accuracy", value=f"{str(round(model.score(x,y), 4))}")
        
        with col2:
            st.metric(label="R2 score", value=f"{str(round(score, 4))}")

        with col3:                        
            st.metric(label="Mean Squared Error (MSE)", value=f"{str(round(mean_squared_error(y, y_pred), 4))}")            
